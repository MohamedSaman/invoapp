...
const mockExpenses = [
  { id: 'EXP001', description: 'Office Supplies', category: 'Supplies', amount: 5000, date: '2025-01-05', status: 'Paid' },
  { id: 'EXP002', description: 'Rent Payment', category: 'Rent', amount: 50000, date: '2025-01-01', status: 'Paid' },
  { id: 'EXP003', description: 'Utilities', category: 'Utilities', amount: 8500, date: '2024-12-28', status: 'Pending' },
];

export default function ListExpensesScreen() {
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.sectionHeader}>
          ...
          <View style={styles.headerText}>
            ...
            <Text style={styles.subtitle}>View all expenses</Text>
          </View>
        </View>
          ...
          <ScrollView horizontal={true} showsHorizontalScrollIndicator={false}>
            <View style={styles.table}>
              <View style={styles.tableHeader}>
                ...
                <Text style={[styles.tableHeaderCell, { width: 100 }]}>EXPENSE ID</Text>
                <Text style={[styles.tableHeaderCell, { width: 150 }]}>DESCRIPTION</Text>
                <Text style={[styles.tableHeaderCell, { width: 120 }]}>CATEGORY</Text>
                <Text style={[styles.tableHeaderCell, { width: 100 }]}>AMOUNT (Rs.)</Text>
              ...
              {filteredData.map((expense, index) => (
                <View key={expense.id} style={[styles.tableRow, index % 2 === 0 && styles.tableRowAlt]}>
                  <Text style={[styles.tableCell, { width: 50 }]}>{index + 1}</Text>
                  <Text style={[styles.tableCell, { width: 100 }]}>{expense.id}</Text>
                  <Text style={[styles.tableCell, { width: 150 }]}>{expense.description}</Text>
                  <Text style={[styles.tableCell, { width: 120 }]}>{expense.category}</Text>
                  <Text style={[styles.tableCell, { width: 100 }]}>
                    {expense.amount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                  </Text>
                  <Text style={[styles.tableCell, { width: 120 }]}>{expense.date}</Text>
